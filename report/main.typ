...
Il rango $i$ riceve l'intervallo $[ |E| i slash p, |E| (i+1) slash p)$ tramite `edge_begin_for_rank`/`edge_end_for_rank` (`mpi/main.cpp:38-44`). Ogni processo calcola i candidati locali sulla propria porzione di archi e poi chiama `MPI_Allreduce` con operazione `MIN` su esattamente $|V|$ chiavi `uint64_t` impacchettate (`reduce_minima`, `mpi/main.cpp:109-118`). La contract usa `mst::dsu::disjoint_set<uncompressed_parents>` (`mpi/main.cpp:176`), cioè la DSU sequenziale con union-by-size e compressione completa.

Lo scostamento dall'ottimo teorico nasce dalla collettiva: anche con pochi processi, la `MPI_Allreduce` comunica $|V|$ chiavi per round e introduce il termine $|V| dot log p$ di @eq:mpi-time indipendentemente da quanto la scan locale sia rapida -- un costo fisso per round che il modello imputa alla libreria, non al codice applicativo. Una funzione di supporto, `broadcast_parents` (`mpi/main.cpp:88-96`), prepara una trasmissione dello snapshot dei parent ma non è mai invocata nel ciclo round (`main`, righe 188-213): il comportamento osservato coincide comunque con il modello -- nessuna comunicazione esplicita del parent è necessaria, la coerenza tra le copie locali emerge dal replay deterministico delle stesse fusioni -- ma la funzione resta codice morto.
