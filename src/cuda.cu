...
//
// Build:   nvcc -O3 -std=c++14 -arch=sm_70 -o boruvka boruvka_cuda.cu
// Run:     ./boruvka [num_vertices] [num_edges] [seed]
...
__host__ __device__ inline u64 pack_key(float w, u32 edge_idx) {
    return (static_cast<u64>(float_to_orderable(w)) << 32) |
           static_cast<u64>(edge_idx);
}
...
__global__ void k_reset_min(u64* min_edge, int n) {
    ...
    if (i < n) min_edge[i] = KEY_SENTINEL;
}
...
__global__ void k_find_min_edges(const int* src, const int* dst, const float* w,
    ...
    int cu = comp[src[e]];
    int cv = comp[dst[e]];
    if (cu == cv) return;                         // internal edge: ignore
...
__global__ void k_build_successor(const int* src, const int* dst,
                                  const int* comp, const u64* min_edge,
                                  int* successor, int n) {
    int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= n) return;
    u64 key = min_edge[c];
    if (key == KEY_SENTINEL) { successor[c] = c; return; }
...
__global__ void k_mark_and_break(const u64* min_edge, const int* succ,
                                 int* nsucc, char* in_mst, int* added, int n) {
    int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= n) return;
    int s = succ[c];
    nsucc[c] = s;                       // default: keep the link
    if (s == c) return;                 // root / isolated: nothing to add
...
__global__ void k_jump(const int* in, int* out, char* changed, int n) {
    ...
    int ss = in[s];
    out[c] = ss;
    if (ss != s) *changed = 1;          // not yet fully flattened
}
...
// Host driver
// ----------------------------------------------------------------------------
static inline int grid(int n, int block) { return (n + block - 1) / block; }

struct GpuResult { std::vector<char> in_mst; int rounds; };

GpuResult boruvka_gpu(int nv, const std::vector<int>& h_src,
                      const std::vector<int>& h_dst,
    ...
    while (true) {
        ...
        k_mark_and_break<<<grid(nv,B), B>>>(d_min, d_succ, d_nsucc,
                                            d_in_mst, d_added, nv);
        int added = 0;
        CUDA_CHECK(cudaMemcpy(&added, d_added, sizeof(int), cudaMemcpyDeviceToHost));
        if (added == 0) break;                // no inter-component edge left
    ...
    cudaFree(d_src); cudaFree(d_dst); cudaFree(d_w); cudaFree(d_comp);
    cudaFree(d_succ); cudaFree(d_nsucc); cudaFree(d_root); cudaFree(d_min);
    cudaFree(d_in_mst); cudaFree(d_added); cudaFree(d_changed);
    return res;
}
...
double kruskal_weight(int nv, const std::vector<int>& s,
                      const std::vector<int>& d, const std::vector<float>& w,
                      int& edges_out) {
    ...
    DSU dsu(nv);
    double total = 0.0; int e = 0;
    for (int i : idx)
        if (dsu.unite(s[i], d[i])) { total += w[i]; ++e; }
    edges_out = e;
    return total;
}
...
// ----------------------------------------------------------------------------
void gen_graph(int nv, int target_m, unsigned seed,
               std::vector<int>& s, std::vector<int>& d, std::vector<float>& w) {
    ...
    while (static_cast<int>(s.size()) < target_m) { // extra random edges
        ...
        if (a == b) continue;
        s.push_back(a); d.push_back(b); w.push_back(wd(rng));
    }
}

int main(int argc, char** argv) {
    int nv      = (argc > 1) ? std::atoi(argv[1]) : 100000;
    int m       = (argc > 2) ? std::atoi(argv[2]) : 1000000;
    unsigned sd = (argc > 3) ? static_cast<unsigned>(std::atoi(argv[3])) : 42u;
    if (m < nv - 1) m = nv - 1;

    std::vector<int> s, d; std::vector<float> w;
    ...
    float ms = 0.f; CUDA_CHECK(cudaEventElapsedTime(&ms, t0, t1));

    double gpu_w = 0.0; int gpu_e = 0;
    for (size_t i = 0; i < r.in_mst.size(); ++i)
        if (r.in_mst[i]) { gpu_w += w[i]; ++gpu_e; }

    int cpu_e = 0;
    double cpu_w = kruskal_weight(nv, s, d, w, cpu_e);

    printf("GPU Boruvka : weight=%.6f  edges=%d  rounds=%d  time=%.3f ms\n",
           gpu_w, gpu_e, r.rounds, ms);
