...
#SBATCH --job-name=parallel-mst-build
...
mkdir -p "$EXPERIMENT_DIR/job_logs"
...
cd "$REPO_DIR"
make clean
make -j"${SLURM_CPUS_PER_TASK:-4}" USE_CMAKE=OFF all core_tests \
  CXX=g++ MPICXX=mpicxx NVCC=nvcc NVCC_CCBIN=g++
