...
# Un rank per CPU allocata: massimizza il parallelismo a grana fine di MPI.
resources="${SLURM_NTASKS:-1}"
csv_path="$RESULTS_DIR/mpi_${SLURM_JOB_ID}.csv"
...
# di archi del grafo generato (src/mpi.cpp genera esattamente questo numero
# di archi), quindi density = edges/vertices è la densità esatta.
for edges in "${edges_values[@]}"; do
  printf 'Running MPI vertices=%s edges=%s seed=%s ranks=%s\n' \
    "$RANDOM_VERTICES" "$edges" "$RANDOM_SEED" "$resources"
  out="$(mpirun -np "$resources" ./build/mpi_app "$RANDOM_VERTICES" "$edges" "$RANDOM_SEED")"
  printf '%s\n' "$out"
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    mpi "$RANDOM_VERTICES" "$edges" "$density" "$RANDOM_SEED" "$resources" \
